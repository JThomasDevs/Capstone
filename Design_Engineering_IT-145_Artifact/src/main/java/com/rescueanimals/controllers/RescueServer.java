...
public class RescueServer {
    ...
    public static void main(String[] args) {
        ...
        // List endpoints
        app.get("/api/dogs", ctx -> ctx.json(controller.listDogs()));
        app.get("/api/monkeys", ctx -> ctx.json(controller.listMonkeys()));
        app.get("/api/available", ctx -> ctx.json(controller.listAvailable()));

        // Add endpoints
        app.post("/api/dogs", RescueServer::addDog);
        app.post("/api/monkeys", RescueServer::addMonkey);

        // Reserve endpoint
        app.post("/api/reserve/{type}/{name}", RescueServer::reserveAnimal);
     ...
     */
    private static void addDog(Context ctx) {
        try {
            Dog dog = gson.fromJson(ctx.body(), Dog.class);
            boolean success = controller.addDog(dog);
     ...
     * @param ctx Javalin HTTP context
     */
    private static void addMonkey(Context ctx) {
        try {
            Monkey monkey = gson.fromJson(ctx.body(), Monkey.class);
            boolean success = controller.addMonkey(monkey);
            ctx.json(new StatusResponse(success));
        } catch (JsonSyntaxException | NullPointerException e) {
            ctx.status(400).json(new StatusResponse(false));
        }
